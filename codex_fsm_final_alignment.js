...
console.log('RECONSTRUINDO O PROJETO SEGUINDO O PLAYBOOK ORIGINAL...');

const appPath = path.join(__dirname, 'app.js');

const playbookAppJs = `
function injectPlaybookStyle() {
    if (document.getElementById('fsm-playbook-style')) return;
    const style = document.createElement('style');
    style.id = 'fsm-playbook-style';
    style.innerHTML = \`
...
function analisarLaudoForense(d) {
    return {
        id: d.os_numero || 'S/N',
...
function atualizarWidgets(base) {
    const faturamento = base.reduce((a, b) => a + b.valor, 0);
    const perda = base.reduce((a, b) => a + b.perda, 0);
    const faturaveis = base.filter(b => b.status === 'FATURÁVEL AVULSO').length;
    const alertas = base.filter(b => b.status !== 'FATURÁVEL AVULSO').length;
    const map = {
...
function renderPainel(base) {
    ...
    const faturamento = base.reduce((a, b) => a + b.valor, 0);
    const perda = base.reduce((a, b) => a + b.perda, 0);
    const porStatus = group(base, b => b.status);
    host.innerHTML = \`
        <section>
            <h2>FSM 360 - Painel do Dia</h2>
            <div class="fsm-grid">
                <div class="card-fsm"><span class="fsm-muted">Faturamento Real</span><h3>\${moeda(faturamento)}</h3></div>
                <div class="card-fsm"><span class="fsm-muted">Dreno Real de Margem</span><h3 style="color:#ef4444">\${moeda(perda)}</h3></div>
                <div class="card-fsm"><span class="fsm-muted">OS Processadas</span><h3>\${base.length}</h3></div>
            </div>
            <div class="card-fsm">
                <h3>Gavetas Financeiras</h3>
    ...
    host.innerHTML = \`
        <section>
            ...
            <div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:12px;">
                ...
                <button class="btn-fsm" onclick="filtrar('REINCIDÊNCIA TÉCNICA')">Reincidências</button>
                <button class="btn-fsm" onclick="filtrar('FATURÁVEL AVULSO')">Faturáveis</button>
                <button class="btn-fsm" onclick="filtrar('PREVENTIVA (CONTRATO)')">Contrato</button>
            ...
            <table class="fsm-table">
                ...
                <tbody>
                    \${filtered.slice(0, 120).map(item => \`
                        <tr>
                            <td><b>\${item.id}</b></td>
                            <td>\${String(item.cliente).substring(0, 38)}</td>
                            <td><span class="tag \${item.statusUi}">\${item.status}</span></td>
                            <td>\${item.causa}</td>
                            <td style="color:\${item.valor > 0 ? '#10b981' : '#94a3b8'}">\${moeda(item.valor)}</td>
                            <td><span class="fsm-muted">\${item.regra}</span></td>
                        </tr>
                    \`).join('')}
                </tbody>
            </table>
...
function renderAnaliseTecnica(base) {
    ...
    host.innerHTML = \`
        <section>
            ...
            <div class="card-fsm">
                <h3>Defeito vs. Peça - Pareto de Componentes</h3>
...
function renderTecnicos(base) {
    ...
    base.forEach(item => {
        ...
        if (!techs[tecnico]) techs[tecnico] = { total: 0, primeira: 0, perda: 0, faturamento: 0 };
        techs[tecnico].total += 1;
        techs[tecnico].faturamento += item.valor;
        techs[tecnico].perda += item.perda;
        if (item.status !== 'REINCIDÊNCIA TÉCNICA') techs[tecnico].primeira += 1;
    });
    host.innerHTML = \`
        <section>
            <h2>Técnicos - Linguagem de Coordenador</h2>
            <div class="fsm-grid">
                \${Object.entries(techs).sort((a,b)=>b[1].perda-a[1].perda).map(([nome, t]) => {
                    ...
                    return \`
                        <div class="card-fsm">
                            ...
                            <div class="fsm-muted" style="margin-top:8px;">\${t.total} OS | Retrabalho \${moeda(t.perda)} | Receita \${moeda(t.faturamento)}</div>
                        </div>
                    \`;
                }).join('')}
            </div>
...
function renderClientes(base) {
    ...
    base.forEach(item => {
        ...
        if (!clients[cliente]) clients[cliente] = { total: 0, perda: 0, faturamento: 0, infra: 0 };
        clients[cliente].total += 1;
        clients[cliente].perda += item.perda;
        clients[cliente].faturamento += item.valor;
        if (String(item.causa).includes('Infra') || String(item.causa).includes('Rede')) clients[cliente].infra += 1;
    });
    host.innerHTML = \`
        <section>
            <h2>Clientes - Saúde Contratual</h2>
            <p class="fsm-muted">Zonas de atrito onde a operação perde margem ou a infraestrutura do cliente consome atendimento.</p>
...
async function renderPlaybookFSM(aba, filtro) {
    const dados = await getDados();
    const base = dados.map(analisarLaudoForense);
    atualizarWidgets(base);
    const target = slug(aba || 'painel-do-dia');
    if (target.includes('operacao')) return renderOperacao(base, filtro);
    if (target.includes('analise')) return renderAnaliseTecnica(base);
    if (target.includes('tecnico')) return renderTecnicos(base);
    if (target.includes('cliente')) return renderClientes(base);
    return renderPainel(base);
}

window.filtrar = function(filtro) {
    renderPlaybookFSM('operacao', filtro);
};
...
window.switchTab = function(aba) {
    ...
    renderPlaybookFSM(aba).catch(err => {
        console.error('Falha no Playbook FSM:', err);
        ensureRoot().innerHTML = '<div class="card-fsm">Erro ao carregar Playbook FSM: ' + err.message + '</div>';
    });
};
...
fs.writeFileSync(appPath, playbookAppJs, 'utf8');
console.log('Projeto alinhado ao Playbook. Governanca, Tecnica e Financeiro em harmonia.');
